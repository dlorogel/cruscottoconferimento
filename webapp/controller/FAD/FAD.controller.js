sap.ui.define([
    ...
    function (Controller,
        ...
        return Controller.extend("it.orogel.cruscottoconferimento.controller.FAD.FAD", {
            formatter: formatter,
            onInit: function () {
                ...
                oRouter.getRoute("FAD.FAD").attachMatched(this._onObjectMatched, this);
            ...
            _onObjectMatched: function () {
                ...
                this.getView().byId("iLIFNR").removeAllTokens();
                                        ...
                                        if (sAction == 'OK') {
                                            if (aError3.length > 0) {
                                                ...
                                                if (aNoError.length > 0) {
                                                    ...
                                                    oRouter.navTo("FAD.Compensazione");
                                                } else {
            ...
            onValueHelpRequest: async function (dynamicModel, event, field) {// [Gheorghe]
                ...
                if (!this._pValueHelpDialog) {
                    this._pValueHelpDialog = Fragment.load({
                        id: oView.getId(),
                        name: "it.orogel.cruscottoconferimento.view.FAD.Fragments.DynamicFrag" + Modello,
                        controller: this
                ...
                if (TypeField !== "StatoA" || oAppModel.getProperty("/StatoDa")) {
                    ...
                    if (!this._statiValueHelpDialog) {
                        this._statiValueHelpDialog = Fragment.load({
                            id: oView.getId(),
                            name: "it.orogel.cruscottoconferimento.view.FAD.Fragments.StatiFrag",
                            controller: this
                        }).then(function (oValueHelpDialog) {
                            oView.addDependent(oValueHelpDialog);
                            return oValueHelpDialog;
            ...
            onValueHelpText: async function (event, TypeField) {
                ...
                if (!this._textValueHelpDialog) {
                    this._textValueHelpDialog = Fragment.load({
                        id: oView.getId(),
                        name: "it.orogel.cruscottoconferimento.view.FAD.Fragments.DynamicTable",
                        controller: this
                    }).then(function (oValueHelpDialog) {
                        oView.addDependent(oValueHelpDialog);
                        return oValueHelpDialog;
            ...
            onValueHelpTextNotaCredito: async function (event, TypeField) {
                ...
                if (!this._textValueNotaCreditoHelpDialog) {
                    this._textValueNotaCreditoHelpDialog = Fragment.load({
                        id: oView.getId(),
                        name: "it.orogel.cruscottoconferimento.view.FAD.Fragments.DynamicTableNotaCredito",
                        controller: this
                    }).then(function (oValueHelpDialog) {
                        oView.addDependent(oValueHelpDialog);
                        return oValueHelpDialog;
            ...
            onValueHelpTextCambioRegimeFiscale: async function (event, TypeField) {
                ...
                if (!this._textValueCambioRegimeFiscaleHelpDialog) {
                    this._textValueCambioRegimeFiscaleHelpDialog = Fragment.load({
                        id: oView.getId(),
                        name: "it.orogel.cruscottoconferimento.view.FAD.Fragments.DynamicTableCambioRegimeFiscale",
                        controller: this
                    }).then(function (oValueHelpDialog) {
                        oView.addDependent(oValueHelpDialog);
                        return oValueHelpDialog;
            ...
            onValueHelpRequestConferimento: async function (dynamicModel, event, field) {// [Gheorghe]
                ...
                if (field === 'BUKRS' || field === 'BUKRS2') {
                    ...
                    var aResults = await this._APIGet(this.getOwnerComponent().getModel(), "/MtartSet", "Chiave,Descrizione")
                    aResults.forEach(e => {
                        var obj = { key: e.Chiave, text: e.Descrizione, type: field }
                        arrayRes.push(obj)
                    });
            ...
            onModificaStato: function (oEvent) {
                ...
                if (!this._ModificaStatoValueHelpDialog) {
                    this._ModificaStatoValueHelpDialog = sap.ui.xmlfragment("it.orogel.cruscottoconferimento.view.FAD.Fragments.ModificaStato", this);
            ...
            onInvioXML: function (oEvent) {
                ...
                const oPromiseRows = oSelIndices.map(item => {
                    ...
                    //                        RowsFiltered = Rows.filter(x => parseFloat(x.Zidrif) !== 0 && Row.Zidrif === x.Zidrif && parseFloat(Row.Zidstorico) < parseFloat(x.Zidstorico) && x.Zstatus !== "C");
                                ...
                                if (!Forfettario) {
                                        ...
                                        if (aFilter.length > 0) {
                                            ...
                                            aFilter.forEach(x => {
                                                if (x.Repos) {
                                                    ...
                                                    //sAllegato1 = sAllegato1.replace("{Price}", ImportoRiga.toFixed(4));
                                                    ...
                                                    sommaQuantitaRiga = parseInt(sommaQuantitaRiga) + parseInt(x.Menge);
                                                } else {
                                                    sAllegato1 = sAllegato1.replace("{ROW1TABLE1}", Constants.FATTURA.ROW1TABLE1);
                                            ...
                                            sommaQuantita = parseInt(sommaQuantita) + parseInt(sommaQuantitaRiga);
                                    ...
                                    if (Fattura.Bukrs === "IT04") {
                                        aPesoLordo.forEach(y => {
                                            ...
                                            //sAllegato1 = sAllegato1.replace("{GrossKg}", parseFloat(y.PesoLordo).toFixed(2));
                                            ...
                                            //sAllegato1 = sAllegato1.replace("{TotalAmount}", parseFloat(AmountPesoLordo).toFixed(2));
                                    ...
                                    //IMPORTI FATTURA
                                    //sAllegato1 = sAllegato1.replace("{VTotaleMerceConferita}", parseFloat(sommaTotale).toFixed(2));
                                    ...
                                    Imponibili.forEach(x => {
                                        aUniqueZiva.forEach(iva => {
                                            ...
                                            if (parseFloat(ZtotIva) !== 0 && Percentage === x.ziva) {
                                                ...
                                                //sAllegato1 = sAllegato1.replace("{Imponibile}", parseFloat(x.importo).toFixed(2));
                ...
                if (Rows.length === 1) {
                    if (Rows[0].ZRilCont && Rows[0].Ztotdoc > 0) {
                        ...
                        if (!this._searchHelpNotaCreditoDialog) {
                            this._searchHelpNotaCreditoDialog = sap.ui.xmlfragment("it.orogel.cruscottoconferimento.view.FAD.Fragments.NotaCredito", this);
                ...
                if (Rows.length === 1) {
                    if (Rows[0].ZRilCont && Rows[0].Ztotdoc > 0 && Rows[0].Zprogsdi !== "" && Rows[0].Zstatus === "I" && Rows[0].Znumprot !== "") {
                        ...
                        if (!this._searchHelpCambioRegimeFiscaleDialog) {
                            this._searchHelpCambioRegimeFiscaleDialog = sap.ui.xmlfragment("it.orogel.cruscottoconferimento.view.FAD.Fragments.CambioRegimeFiscale", this);
